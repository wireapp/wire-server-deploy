...
If you think you might run into storage issues. Remove the `/opt/assets/containers-helm` directory as we will copy over new ones in the following steps.
...
As of wire-server version 5.0.0. we have moved out webapp, team-settings and account-pages as a standalone deployments/charts.
...
```
d helm upgrade webapp charts/webapp -f values/webapp/values.yaml
```
...
```
d helm upgrade team-settings charts/team-settings -f values/team-settings/values.yaml -f values/team-settings/secrets.yaml
```
...
```
d helm upgrade account-pages charts/account-pages -f values/account-pages/values.yaml
```
