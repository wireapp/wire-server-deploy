...
function list-debian-packages() {
  ...
  find debs-jammy/pool/ -type f -name "*.deb" | while read -r pkg; do
    ...
    jq --arg name "$name" --arg version "$version" \
      '. += [{ name: $name, version: $version }]' debian-builds.json > debian-builds.tmp && mv debian-builds.tmp debian-builds.json
  done
}
...
wire_build="https://raw.githubusercontent.com/wireapp/wire-builds/eb05a996d869484ac77aad7fdee84d2c6c4bb42e/build.json"
...
tar czf assets.tgz debs-jammy.tar binaries.tar containers-adminhost containers-helm.tar containers-system.tar ansible charts values bin debian-builds.json
